  ...
  const handleAcceptTrade = (listing) => {
    if (listing.isSystem) {
      ...
      if (!Number.isNaN(tradeId)) {
        ...
        const currentTrades = recentTrades.filter(
  ...
  return (
    <div className="cyber-font relative flex h-screen bg-[#0a0c10] text-slate-300 overflow-hidden cyber-grid">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
          <div className="w-full max-w-md bg-[#161b22] border border-[#00d4ff]/30 p-8 relative neon-glow">
            <button
              ...
              className="absolute top-4 right-4 text-slate-500 hover:text-[#00d4ff]"
            >
              <X size={20} />
            </button>
            <h2 className="text-xl font-bold text-[#00d4ff] mb-6 uppercase tracking-widest italic">
              Broadcast New Trade
            </h2>
            <form onSubmit={handleAddItem} className="space-y-4">
              ...
              <input
                required
                placeholder="SEEKING (e.g. Nanobots)"
                className="w-full bg-black/40 border border-[#30363d] p-3 text-sm focus:border-[#00d4ff] outline-none"
              ...
              <button
                type="submit"
                className="w-full bg-[#00d4ff] text-black font-black py-3 uppercase tracking-widest hover:brightness-110"
              >
                Execute Listing
              </button>
            </form>
      ...
      <aside className="w-64 border-r border-[#30363d] bg-[#0a0c10] flex flex-col p-6 z-20">
        <div className="mb-10">
          <h1 className="text-[#00d4ff] font-bold text-xl tracking-tighter leading-none">
            NEO
          </h1>
          <h1 className="text-[#00d4ff] font-bold text-2xl tracking-tighter leading-none uppercase">
            JAMSHEDPUR
          </h1>
        ...
        <div className="flex items-center gap-3 p-3 bg-[#161b22]/50 border border-[#30363d] rounded mb-8">
          ...
          <span className="text-[9px] border border-[#00d4ff] text-[#00d4ff] px-2 py-0.5 rounded">
            CITIZEN
          </span>
        </div>
        ...
        <div className="pt-4 border-t border-[#30363d] text-[10px] text-slate-600">
          <button
            ...
            className="flex items-center gap-2 uppercase tracking-widest text-left text-[10px] opacity-50 hover:opacity-100 transition-all p-2"
          >
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <div className="h-14 border-b border-[#30363d] flex items-center justify-between px-8 bg-black/20">
          <span className="text-[10px] text-slate-600 tracking-[0.2em] uppercase">
            Neo-Jamshedpur Unified City Platform
          </span>
        ...
        <div className="flex-1 overflow-y-auto p-10 custom-scrollbar">
          <header className="flex justify-between items-end mb-10">
            <div>
              <p className="text-[10px] text-slate-500 tracking-[0.3em] uppercase mb-1">
                Resource Exchange
              </p>
            ...
            {listings
              ...
              .map((item) => (
                <TradeCard
                  key={item.id}
                  {...item}
                  onAccept={() => handleAcceptTrade(item)}
                />
              ))}
          </div>
        </div>
...
const SidebarLink = ({ icon, label, sub, active, onClick }) => (
  <div
    onClick={onClick}
    className={`flex items-center gap-4 p-2 cursor-pointer border-r-2 transition-all ${active ? "text-[#00d4ff] border-[#00d4ff] bg-[#00d4ff]/5" : "text-slate-500 border-transparent hover:text-slate-300"}`}
  >
    {icon}
    <div>
...
const StatCard = ({ label, value }) => (
  <div className="bg-[#161b22]/30 border border-[#30363d] p-6 text-center group hover:border-cyan-500/30 transition-all">
    <p className="text-4xl font-bold text-white group-hover:text-cyan-400 transition-colors">
      {value}
    </p>
  ...
  <div className="bg-[#161b22]/20 border border-[#30363d] p-6 hover:bg-[#161b22]/40 transition-all border-l-2 border-l-[#30363d] hover:border-l-[#00d4ff] animate-in fade-in slide-in-from-bottom-2 duration-300">
    <div className="flex justify-between mb-4">
      <div className="flex gap-2">
        <span className="text-[9px] bg-cyan-900/20 text-cyan-500 px-2 py-0.5 rounded border border-cyan-500/30 font-bold uppercase tracking-tighter">
          Verified
        </span>
        <span className="text-[9px] text-slate-500 uppercase py-0.5">
          {category}
        </span>
      </div>
      <span className="text-[9px] text-[#00d4ff] border border-[#00d4ff]/30 px-2 py-0.5 font-bold">
        AVAILABLE
      </span>
    </div>
    <h3 className="font-bold text-lg text-slate-200 mb-2">{title}</h3>
    <p className="text-[13px] text-slate-500 mb-6 leading-relaxed line-clamp-2">
      {desc}
    </p>
    <div className="flex justify-between items-end border-t border-[#30363d] pt-4">
      <div className="flex-1 overflow-hidden">
        ...
        <p className="text-[13px] text-[#00d4ff] font-semibold truncate">
          {seeking}
        </p>
      ...
      <div className="text-right ml-4 min-w-15">
        ...
        <p className="text-[12px] text-yellow-500 font-bold whitespace-nowrap">
          ⭐ {rep} REP
        </p>
      </div>
    </div>
    <button
      onClick={onAccept}
      className="mt-6 w-full bg-[#00d4ff] text-black font-bold uppercase text-[10px] py-3 tracking-widest hover:brightness-110"
    >
      ACCEPT TRADE
    </button>
  </div>
