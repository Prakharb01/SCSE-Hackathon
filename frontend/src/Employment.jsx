...
const EmploymentGrid = () => {
  ...
  const handleAcceptJob = () => {
    if (selectedJob) {
      ...
      setRecentJobs((prev) =>
        prev.filter((job) => job.title !== selectedJob.title),
      );
      ...
      <aside className="w-64 border-r border-cyan-900/40 bg-[#030c0f] flex flex-col p-4 shrink-0">
        ...
        <div className="flex items-center gap-3 p-3 bg-cyan-950/20 rounded border border-cyan-900/30 mb-6">
          ...
          <div className="flex-1">
            ...
            <div className="text-[9px] text-cyan-400 opacity-70 italic tracking-tighter">
              {currentUser?.Ranking || 0} REP
            </div>
          </div>
          <span className="text-[8px] border border-cyan-500/50 px-1 text-cyan-400 bg-cyan-500/10">
            CITIZEN
          </span>
        </div>
        ...
        <div className="mt-auto space-y-4">
          <button
            ...
            className="flex items-center gap-2 text-[10px] opacity-50 hover:opacity-100 transition-all p-2"
          >
            <LogOut size={14} /> DISCONNECT
          </button>
          <div className="text-[9px] opacity-30 px-2 uppercase tracking-widest">
            Sector: Tech Quarter
          </div>
        </div>
      </aside>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
            <div className="bg-[#030c0f] border border-cyan-500 w-full max-w-md p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-white font-bold italic uppercase">
                  Broadcast New Contract
                </h3>
                <X
              ...
              <form onSubmit={handleCreateJob} className="space-y-4">
                <input
                  ...
                  className="w-full bg-[#041217] border border-cyan-900 p-2 text-xs outline-none text-white"
                  onChange={(e) =>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <select
                    className="bg-[#041217] border border-cyan-900 p-2 text-[10px] text-cyan-400"
                    onChange={(e) =>
                ...
                <input
                  ...
                  className="w-full bg-[#041217] border border-cyan-900 p-2 text-xs text-yellow-500"
                  onChange={(e) =>
                ...
                <textarea
                  ...
                  rows="3"
                  className="w-full bg-[#041217] border border-cyan-900 p-2 text-xs text-white"
                  onChange={(e) =>
                    setFormData({ ...formData, description: e.target.value })
                  }
                />
                <button className="w-full bg-cyan-400 text-black py-2 font-bold uppercase text-[10px]">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
            <div className="bg-[#030c0f] border border-cyan-500 w-full max-w-md p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setJobOptionsOpen(false)}>
                  <X size={20} />
                </button>
              </div>
              <div className="mb-6">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 bg-cyan-500 text-black font-bold py-3 text-xs"
                >
                  ACCEPT JOB
                </button>
                <button
                  onClick={() => setJobOptionsOpen(false)}
                  className="flex-1 border border-cyan-500 text-cyan-400 font-bold py-3 text-xs"
                >
                  DECLINE
                </button>
              </div>
...
const NavItem = ({ icon, label, sub, active, onClick }) => (
  <div
    onClick={onClick}
    className={`flex items-center gap-3 p-3 cursor-pointer transition-all border-r-2 ${active ? "bg-cyan-950/40 border-cyan-400 text-cyan-300" : "border-transparent opacity-50 hover:opacity-100 hover:bg-cyan-950/10"}`}
  >
    {icon}
    <div>
...
const CounterBox = ({ val, label }) => (
  <div className="bg-[#041217]/50 border border-cyan-900/30 p-4 text-center group hover:border-cyan-500/50 transition-colors">
    <div className="text-3xl font-bold text-cyan-400 leading-none mb-2">
      {val}
    </div>
    <div className="text-[9px] text-cyan-700 tracking-[0.2em] font-bold uppercase">
      {label}
    </div>
  </div>
);
...
const JobCard = ({ type, category, title, description, reward, onClick }) => {
  ...
  return (
    <div
      className="bg-[#041217]/40 border border-cyan-900/30 p-6 relative group hover:bg-[#041217]/80 hover:border-cyan-500/40 transition-all cursor-pointer"
      onClick={onClick}
    >
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-3">
          <span
            className={`text-[8px] px-2 py-0.5 border font-bold ${typeStyles[type] || typeStyles.NORMAL}`}
          >
            {type}
          </span>
          <span className="text-[10px] text-cyan-700 uppercase tracking-widest font-bold">
            {category}
          </span>
        </div>
        <div className="flex flex-col items-end">
          <span className="text-[9px] border border-cyan-400 px-1 text-cyan-400 font-bold scale-90 italic">
            ACTIVE
          </span>
        </div>
      </div>
      ...
      <p className="text-xs text-cyan-100/40 leading-relaxed mb-4 max-w-2xl">
        {description}
      </p>
      <div className="flex justify-between items-center">
        <div className="flex gap-6 text-[10px]">
          <span className="flex items-center gap-2 opacity-60">
            <Calendar size={12} /> 4/18/2026
          </span>
          <span className="flex items-center gap-2 opacity-60">
            <Users size={12} /> 0 APPLICANTS
          </span>
          <span className="text-yellow-500 font-bold uppercase tracking-widest italic">
            Reward: {reward}
          </span>
        </div>
        <ChevronRight
          size={18}
          className="opacity-20 group-hover:opacity-100 transition-all group-hover:translate-x-1"
        />
      </div>
    </div>
  );
