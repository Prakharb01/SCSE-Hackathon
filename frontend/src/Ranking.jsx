    ...
    <div className="cyber-font cyber-bg flex h-screen text-slate-300 overflow-hidden">
      ...
      <aside className="w-64 border-r border-[#30363d] bg-[#0a0c10] flex flex-col p-6 z-20">
        <div className="mb-10">
          <div className="flex items-center gap-2 text-[#00d4ff]">
            ...
            <h1 className="font-bold text-xl leading-none tracking-tighter uppercase">
              Neo JSR
            </h1>
          </div>
        ...
        <div className="pt-4 border-t border-[#30363d] text-[10px] text-slate-600">
          <button
            ...
            className="uppercase tracking-widest flex items-center gap-2 text-left p-2 text-[10px] opacity-50 hover:opacity-100 transition-all"
          >
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          ...
          {["COMMAND", "EMPLOYMENT", "PULSE", "IDENTITY"].includes(
            activePage,
          ) && (
...
const ExchangePage = () => {
  ...
  const [listings, setListings] = useState([
    {
      id: 1,
      title: "Rare Isotope Fuel Cell",
      category: "Energy",
      seeking: "Medical nanobots",
      rep: 100,
    },
    {
      id: 2,
      title: "Encryption Module v7.3",
      category: "Technology",
      seeking: "Biometric scanner",
      rep: 100,
    },
  ]);
  ...
  return (
    <div className="p-10">
      <header className="flex justify-between items-end mb-10">
        <div>
          <p className="text-[10px] text-[#00d4ff] tracking-[0.3em] uppercase mb-1 font-bold">
            Resource Exchange
          </p>
          <h2 className="text-4xl font-bold text-white uppercase italic neon-text">
            Trade Market
          </h2>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="bg-[#00d4ff] text-black font-black px-6 py-2.5 flex items-center gap-2 hover:shadow-[0_0_15px_#00d4ff]"
        >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {listings.map((item) => (
          <div
            key={item.id}
            className="bg-[#161b22]/40 border border-[#30363d] p-6 border-l-2 border-l-[#00d4ff]"
          >
...
const RankingsPage = () => {
  ...
  const sortedUsers = [...users].sort(
    (a, b) => b.credit - a.credit || b.rep - a.rep,
  );

  return (
    <div className="p-12 max-w-4xl mx-auto">
      <header className="text-center mb-12">
        <h2 className="text-5xl font-black text-white uppercase italic neon-text">
          Leaderboard
        </h2>
        <p className="text-xs text-slate-500 mt-2 tracking-widest uppercase">
          Top Citizens by Credit & Reputation
        </p>
      </header>
      <div className="space-y-4">
        {sortedUsers.map((u, i) => (
          <div
            key={u.name}
            className={`flex items-center p-4 bg-[#161b22]/40 border ${u.isUser ? "border-[#00d4ff]" : "border-[#30363d]"}`}
          >
            ...
            <div className="text-right">
              ...
              <p className="text-[8px] uppercase text-slate-500 mt-2">
                Reputation
              </p>
