    ...
    <div className="flex min-h-screen bg-[#02080a] text-[#00e5ff] font-mono selection:bg-cyan-500/30">
      ...
      <aside className="w-64 border-r border-cyan-900/50 bg-[#030c0f] flex flex-col p-4">
        <div className="flex items-center gap-2 mb-8 px-2">
          ...
          <h1 className="text-lg font-bold tracking-tighter uppercase leading-tight">
            Neo
            <br />
            Jamshedpur
          </h1>
        ...
        <div className="flex items-center gap-3 p-3 bg-cyan-950/20 rounded-sm border border-cyan-900/30 mb-6">
          ...
          <div>
            ...
            <div className="text-[10px] text-cyan-400 opacity-70 italic">
              {currentUser?.Ranking || reputation} REP • CITIZEN
            </div>
          </div>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-2 text-xs opacity-50 hover:opacity-100 transition-all p-2 mt-auto"
        >
...
const CommandView = ({
  ...
  <div className="animate-in fade-in duration-500">
    <header className="flex justify-between items-start mb-8">
      <div>
        <div className="text-[10px] tracking-[0.2em] text-cyan-500/60 uppercase">
          Command Center
        </div>
        ...
        <div className="text-[10px] mt-1 opacity-70">
          SECTOR: Tech Quarter | CLEARANCE: Citizen
        </div>
      </div>
      <div className="text-right">
        ...
        <div className="text-4xl font-bold text-white">
          {currentUser?.Ranking || reputation}
        </div>
    ...
    <div className="grid grid-cols-4 gap-4 mb-6">
      ...
      <StatCard
        ...
        val={criticalAlerts.toString()}
        sub="CITY EMERGENCIES"
      />
      <StatCard
        icon={<Bell size={16} />}
        label="NOTIFICATIONS"
    ...
    <div
      className={`${criticalAlerts > 0 ? "bg-red-950/20 border-red-900/50 text-red-200" : "bg-cyan-950/10 border-cyan-900/30 text-cyan-300"} p-4 mb-6 flex items-start gap-4`}
    >
      <AlertTriangle
        className={`${criticalAlerts > 0 ? "text-red-500" : "text-cyan-400"} shrink-0`}
        size={20}
      />
      <div>
        <div
          className={`text-xs font-bold uppercase tracking-widest ${criticalAlerts > 0 ? "text-red-500" : "text-cyan-400"}`}
        >
          {criticalAlerts > 0
            ? `Critical City Alerts (${criticalAlerts})`
            : "No Critical Alerts"}
        </div>
        <div
          className={`text-sm mt-1 italic font-bold ${criticalAlerts > 0 ? "text-red-200" : "text-cyan-200"}`}
        >
          {criticalAlertMessage}
        </div>
      </div>
    ...
    <div className="grid grid-cols-2 gap-6 mb-6">
      <SectionBox
        title="RECENT JOBS"
        onViewAll={() => navigate("/app/employment")}
      >
        {recentJobs.map((job) => (
          <JobRow
            key={job.id}
      ...
      <SectionBox
        title="RECENT TRADES"
        onViewAll={() => navigate("/app/exchange")}
      >
    ...
    <SectionBox
      title="CITY ACTIVITY FEED"
      onViewAll={() => navigate("/app/thepulse")}
    >
    ...
    <div className="grid grid-cols-3 gap-4 mt-6">
      ...
      <FooterStat
        ...
        val={completedJobs.toString()}
        label="JOBS COMPLETED"
      />
      <FooterStat
        icon={<RefreshCw size={20} />}
        val={completedTrades.toString()}
...
const EmploymentView = ({ completedJobs }) => (
  <div className="animate-in fade-in duration-500 max-w-5xl mx-auto">
    <div className="flex justify-between items-end mb-8">
      ...
      <button className="bg-cyan-400 text-black px-6 py-2 font-bold text-xs flex items-center gap-2 hover:bg-white transition-all uppercase">
    ...
    <div className="space-y-4">
      ...
      />
      <JobCard
        title="Power Grid Emergency Repair"
        type="CRITICAL"
        sector="Engineering"
        reward="1500 credits"
      />
  ...
  <div className="animate-in fade-in duration-500 max-w-4xl mx-auto">
    ...
    <div className="bg-[#041217]/40 border border-cyan-900/30 p-6">
      <div className="text-[9px] text-cyan-400 font-black mb-2 uppercase">
        ● SOCIAL — Steel District
      </div>
      <h3 className="text-xl font-bold text-white mb-2 italic">
        Black Market Rumors
      </h3>
      <p className="text-xs text-cyan-100/40 mb-4 font-sans leading-relaxed">
        City Watch confirmed three arrests in the old foundry following reports
        of unlicensed trade hubs.
      </p>
      <div className="flex justify-between text-[10px] opacity-40 uppercase pt-4 border-t border-cyan-900/20 font-bold">
        ...
        <div className="flex gap-4">
          <Eye size={12} /> 85 <Heart size={12} /> 12
        </div>
      </div>
    </div>
  </div>
...
const IdentityView = () => (
  <div className="animate-in fade-in duration-500 max-w-4xl mx-auto space-y-8">
    ...
    <div className="bg-[#030c0f] border border-cyan-500/30 p-8 flex items-center gap-8 relative overflow-hidden">
      <div className="absolute top-0 right-0 p-2 text-[10px] text-cyan-500/10">
        DNA-ID: 772-XQ-9
      </div>
...
const NavItem = ({ icon, label, sub, active, onClick }) => (
  ...
  >
    ...
    <div>
      ...
      <div className="text-[9px] opacity-70 tracking-tighter uppercase">
        {sub}
      </div>
    </div>
  </div>
...
const JobCard = ({ title, type, sector, reward }) => (
  <div className="bg-[#041217]/60 border border-cyan-900/30 p-6 hover:border-cyan-400/50 transition-all">
    <div className="flex justify-between text-[10px] mb-2 font-bold uppercase">
      <span className={type === "CRITICAL" ? "text-red-500" : "text-cyan-400"}>
        {type}
      </span>
      <span className="opacity-40">{sector}</span>
    </div>
    <h4 className="text-white font-bold text-lg mb-2">{title}</h4>
    <div className="text-[10px] text-yellow-500 italic font-bold">
      REWARD: {reward}
    </div>
  </div>
);
...
const StatCard = ({ icon, label, val, sub, onClick }) => (
  ...
  >
    ...
    <div
      className={`text-[9px] uppercase ${onClick ? "text-cyan-400 opacity-90" : "opacity-40"}`}
    >
      {sub}
    </div>
  </div>
...
const JobRow = ({ title, sector, level }) => (
  <div className="flex justify-between items-center py-3 border-b border-cyan-900/10 last:border-0">
    ...
    <span
      className={`text-[8px] px-1 font-bold ${level === "CRITICAL" ? "bg-red-500 text-black" : "bg-cyan-500/20 text-cyan-400"}`}
    >
      {level}
    </span>
  </div>
...
const TradeRow = ({ title, type, status }) => (
  <div className="flex justify-between items-center py-3 border-b border-cyan-900/10 last:border-0">
    ...
    <span className="text-[8px] px-1 border border-cyan-500/50 text-cyan-400 font-bold">
      {status}
    </span>
  </div>
);
