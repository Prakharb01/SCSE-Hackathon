...
const ThePulse = () => {
  ...
  const { currentUser, reputation = 100 } = useUserContext() || {};
  // --- STATE FOR CITY FEED ---
  ...
  const [feedItems, setFeedItems] = useState([
    {
      ...
      content:
        "Following recent reports of an unlicensed trading hub operating beneath the old foundry, City Watch spokesperson confirmed three arrests and seizure of contraband tech valued at 45,000 credits. Citizens are reminded that all trades must be registered through the Resource Exchange.",
    ...
    },
    {
      ...
      content:
        "The city tech council has announced that the Neural Interface Integration project has successfully completed Phase 2 trials with a 94.7% success rate. Phase 3 will begin next month, opening 200 voluntary participant slots.",
      author: "Director Steele",
      date: "4/18/2026",
      views: 0,
      likes: 0,
    },
    {
      ...
      content:
        "City Engineering has issued a priority blackout warning for Sectors 5 through 9 from 23:00 to 03:00. Emergency power only. Residents advised to charge all devices, secure perishables, and avoid travel after 22:30.",
      author: "Director Steele",
      date: "4/18/2026",
      views: 0,
      likes: 0,
  ...
  const filteredFeedItems = feedItems.filter((item) => {
    ...
    return [item.title, item.content, item.sector, item.type].some((field) =>
  ...
  const alertMessage =
    alertCount > 0
      ? latestAlert?.title || "A critical alert is active."
      : "No active critical alerts at the moment.";
    ...
    <div className="flex min-h-screen bg-[#02080a] text-[#00e5ff] font-mono selection:bg-cyan-500/30 overflow-hidden">
      ...
      <aside className="w-64 border-r border-cyan-900/40 bg-[#030c0f] flex flex-col p-4 z-20">
        ...
        <div className="flex items-center gap-3 p-3 bg-cyan-950/20 rounded border border-cyan-900/30 mb-6">
          ...
          <span className="text-[8px] border border-cyan-500/50 px-1 text-cyan-400 bg-cyan-500/10 uppercase">
            Citizen
          </span>
        </div>
        ...
        <div className="mt-auto pt-4 border-t border-cyan-900/20">
          <button
            ...
            className="flex items-center gap-2 text-[10px] opacity-50 hover:opacity-100 transition-all p-2 uppercase tracking-widest"
          >
            <LogOut size={14} /> Disconnect
          </button>
          <div className="text-[9px] opacity-30 px-2 mt-2 uppercase tracking-widest">
            Sector: Tech Quarter
          </div>
        </div>
      </aside>
        ...
        <div className="flex-1 overflow-y-auto p-10 custom-scrollbar">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="flex justify-between items-end mb-8">
              <div>
                <div className="text-[10px] tracking-[0.3em] text-cyan-500 opacity-60 uppercase mb-1">
                  City Broadcast
                </div>
            ...
            <div
              className={`${alertCount > 0 ? "bg-red-950/10 border-red-900/50 text-red-200 animate-pulse" : "bg-cyan-950/10 border-cyan-900/30 text-cyan-300"} border p-4 mb-8 flex items-start gap-4`}
            >
              <AlertTriangle
                className={`${alertCount > 0 ? "text-red-500" : "text-cyan-400"} shrink-0`}
                size={18}
              />
              <div>
                <div
                  className={`text-[10px] font-bold uppercase tracking-widest ${alertCount > 0 ? "text-red-500" : "text-cyan-400"}`}
                >
                  {alertCount > 0
                    ? `Critical City Alerts Active (${alertCount})`
                    : "No Active Critical Alerts"}
                </div>
                <div
                  className={`text-sm mt-1 font-bold italic ${alertCount > 0 ? "text-red-200" : "text-cyan-200"}`}
                >
                  {alertMessage}
                </div>
              </div>
...
const NavItem = ({ icon, label, sub, active, onClick }) => (
  <div
    onClick={onClick}
    className={`flex items-center gap-3 p-3 cursor-pointer transition-all border-r-2 ${active ? "bg-cyan-950/40 border-cyan-400 text-cyan-300" : "border-transparent opacity-40 hover:opacity-100 hover:bg-cyan-950/10"}`}
  >
    {icon}
    <div>
      <div className="text-[11px] font-bold uppercase leading-none tracking-tight">
        {label}
      </div>
      <div className="text-[9px] opacity-60 tracking-tighter uppercase">
        {sub}
      </div>
    </div>
  </div>
  ...
  return (
    <div className="bg-[#041217]/40 border border-cyan-900/30 p-6 group hover:bg-[#041217]/60 hover:border-cyan-500/40 transition-all animate-in fade-in slide-in-from-top-4 duration-500">
      <div className="flex items-center gap-3 mb-3">
        <div
          className={`w-1.5 h-1.5 rounded-full animate-pulse ${type === "ALERT" ? "bg-red-500" : "bg-cyan-500"}`}
        />
        <span
          className={`text-[9px] px-2 py-0.5 border font-black uppercase tracking-widest ${typeColors[type]}`}
        >
          {type}
        </span>
        <span className="text-[10px] text-cyan-800 uppercase tracking-widest font-bold">
          {sector}
        </span>
      </div>

      <h3 className="text-lg font-bold text-white mb-3 tracking-tight group-hover:text-cyan-400 transition-colors uppercase italic">
        {title}
      </h3>
      <p className="text-xs text-cyan-100/40 leading-relaxed mb-6 font-sans">
        {content}
      </p>
      ...
      <div className="flex justify-between items-center text-[10px] text-cyan-900 font-bold uppercase tracking-widest pt-4 border-t border-cyan-900/20">
        ...
        <div className="flex gap-4 items-center opacity-60">
          ...
          <span className="flex items-center gap-1 cursor-pointer hover:text-cyan-400">
